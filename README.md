...
静寂 → 天の川が柱として立つ → 塚へ降りそそぐ → 環を抜けて湖を三度めぐる → 火を通って空へ還る、で約36秒。
...
| エージェント | のべ 41 体（並行実装 9、レビュー 4、修正 9、点検 7 ほか） |
| API 呼び出し | 10,514 回 |
| 消費トークン | 処理 計 **約 23.3 億**（内訳: キャッシュ読み出し 22.7 億／キャッシュ書き込み 5,151 万／生成 735 万／新規入力 2.1 万） |
| API 換算コスト | 約 **$1,800**（≈ 27 万円 @150円）。キャッシュを使わなければ $11,800 相当だったので、約 85% を削減 |
| 規模 | 実装 約 17,500 行、設計書 [docs/DESIGN.md](docs/DESIGN.md) 1,650 行 |
